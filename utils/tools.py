...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric, printout=False):
    total_loss = []
    total_mae_loss = []
    total_rmse_loss = []  #lqr added
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            if printout:
                if accelerator is not None:
                    #accelerator.print('Predicted: {},'.format(pred_back) + ' True: {}'.format(true_back) + ' RMSE: {}'.format(rmse_loss))
                    accelerator.print('RMSE: {}'.format(rmse_loss))
                else:
                    #print('Predicted: {},'.format(pred_back) + ' True: {}'.format(true_back) + ' RMSE: {}'.format(rmse_loss))
                    print('RMSE: {}'.format(rmse_loss))

            total_loss.append(loss.item())
            total_mae_loss.append(mae_loss.item())
            total_rmse_loss.append(rmse_loss.item())  # lqr added

    total_loss = np.average(total_loss)
    total_mae_loss = np.average(total_mae_loss)
    total_rmse_loss = np.average(total_rmse_loss)  # lqr added
