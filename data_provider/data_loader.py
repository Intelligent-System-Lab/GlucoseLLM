...
class Dataset_Glucose(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.set_type != 3:
            ...
            cols.remove(self.target)
            cols.remove('ts')
            # These are our additional features - automatically determined
            ...
            if self.set_type == 0:  # train
                ...
                border2 = len(df_raw)
        else: # self.set_type == 3:  # new_test
            df_raw = pd.read_csv(os.path.join(self.root_path, self.data_path))
            
            # 与训练部分保持一致，先设置 additional_features
            cols = list(df_raw.columns)
            cols.remove(self.target)
            cols.remove('ts')
            # 设置 additional_features（这是之前缺少的关键步骤）
            self.additional_features = cols
    ...
    def __len__(self):
        # 3. Adjust seq_len and pred_len if needed
        # If you still encounter issues, consider reducing seq_len and pred_len
        #return len(self.data_x)  # Length is now determined by sliding window
        return len(self.data_x) - self.seq_len - self.pred_len + 1  # Adjust length calculation
